...
void eval_triangle_tgf(COMP_PRECISION *x,struct flt *fault,
		       ...
		       COMP_PRECISION sm[3][3],int *giret,int mode,
		       my_boolean is_self,struct el_par elastic)
{
  ...
  if(is_self)
    ...
    eltst3triadirecttarg(&elastic.lambda, &elastic.shear, &unity,
			 triangle, dslip, dnormal, dx, du,
			 &calc_strain, fstrain);
  elth3triaadap(         &elastic.lambda, &elastic.shear, &unity,
			 triangle, dslip, dnormal, dx, &unity, u0,
			 &calc_strain, strain0, &numfunev);
  ...
  if(calc_strain){
    etracel = (fstrain[0*3+0]+fstrain[1*3+1]+fstrain[2*3+2]) * elastic.lambda;
    sm[0][0] = elastic.mu2 * fstrain[0*3+0] + etracel;
    sm[1][1] = elastic.mu2 * fstrain[1*3+1] + etracel;
    sm[2][2] = elastic.mu2 * fstrain[2*3+2] + etracel;
    /* flip the ordering here from fortran */
    sm[0][1]=sm[1][0] = elastic.mu2 * fstrain[1*3+0];
    sm[0][2]=sm[2][0] = elastic.mu2 * fstrain[2*3+0];
    sm[1][2]=sm[2][1] = elastic.mu2 * fstrain[2*3+1];
  }

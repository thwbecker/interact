/*
  ...
  geometry routines, some elasticity stuff,
...
*/
COMP_PRECISION stddev(COMP_PRECISION sum,
		      COMP_PRECISION sum_sqrd, int n)
{
  double xn,sigma;
  xn = (double)n;
...
/* Mw = f(M_0) */
COMP_PRECISION mwfromm0(COMP_PRECISION M0)
{
  return (M0>0.0)?(0.66666666666666666666666666666667*(log10(M0)-9.05)):(-99.0);
}

COMP_PRECISION bulk_mod_from_G_nu(COMP_PRECISION g, COMP_PRECISION nu)
{
  return 2.0*g*(1.0 + nu)/(3.0*(1.0 - 2.0*nu));
}
COMP_PRECISION nu_from_G_bulk(COMP_PRECISION g, COMP_PRECISION bulk)
{
  return (3.0*bulk - 2.0*g)/(6.0*bulk + 2.0*g);
}
//

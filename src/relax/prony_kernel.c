  ...
  *img = fault[isrc];
  ...
  for(n=0;n <= n_img;n++){
    ...
    if(fabs(wfac) < EPS_COMP_PREC)
      break;
    /* reflection order n carries TWO image families (each with its
       free-surface mirror auto-generated by the half-plane kernel):
       the plain shift, element center y = ysave - 2 n H (depths
       2 n H + [c1, c2]), and, for n >= 1, the surface-mirrored
       shift, element center y = -ysave - 2 n H (depths
       2 n H - [c2, c1]), same slip; the second family was missing
       in earlier versions, see ve_rybicki_antiplane_uz */
...
/*
  ...
  elastic reflection coefficient gam,
    ...
    u_z(x, 0) = (1/pi) { atan((x-xs)/c1) - atan((x-xs)/c2)
                + sum_{n=1}^{n_img} gam^n
                [ atan((x-xs)/(2 n H + c1)) - atan((x-xs)/(2 n H + c2))
                + atan((x-xs)/(2 n H - c2)) - atan((x-xs)/(2 n H - c1)) ] }

  each reflection order n carries TWO image families, at depths
  2 n H + [c1, c2] and 2 n H - [c2, c1]; the second family was
  missing in earlier versions of this machinery (both here and in
  the element sampler and the Nur-Mavko forms below, so the mutual
  checks could not catch it); the corrected form was verified
  against an independent Fourier-domain solution of the layered
  antiplane problem, U(k) proportional to (1-E)(1+Gam E)/(1-Gam E^2)
  with E = exp(-kH) for the through-plate fault, against a
  finite-difference solve, and against the Savage-Prescott /
  Segall (2010, eq. 12.19-type) closed form, to which the
  through-plate case collapses term by term.
  ...
  solution, gam = 1 the fully relaxed (free bottom) limit which
  for a through-plate fault is rigid block motion +/- 1/2, gam = 0
				       ...
				       COMP_PRECISION gam, int n_img)
{
  ...
  for(n=0;n <= n_img;n++){
    ...
    if(n > 0){			/* second image family at 2 n H - [c2, c1] */
      d1 = 2.0*(COMP_PRECISION)n*plate_h - c2;
      d2 = 2.0*(COMP_PRECISION)n*plate_h - c1;
      uz += wfac * (atan2(dx,d1) - atan2(dx,d2));
    }
    wfac *= gam;
  }
			       ...
			       COMP_PRECISION t, int n_img)
{
  ...
  int n;
  COMP_PRECISION wn,fam;
  dx = x - xs;
  uz = 0.0;
  for(n=0;n <= n_img;n++){
    wn = (n == 0)?(1.0):(ve_nur_mavko_weight(n,gamma0,b,t));
    d1 = c1 + 2.0*(COMP_PRECISION)n*plate_h;
    d2 = c2 + 2.0*(COMP_PRECISION)n*plate_h;
    fam = atan2(dx,d1) - atan2(dx,d2);
    if(n > 0){			/* second image family, see ve_rybicki_antiplane_uz */
      d1 = 2.0*(COMP_PRECISION)n*plate_h - c2;
      d2 = 2.0*(COMP_PRECISION)n*plate_h - c1;
      fam += atan2(dx,d1) - atan2(dx,d2);
    }
			       ...
			       COMP_PRECISION t, int n_img)
{
  ...
  for(n=1;n <= n_img;n++){
    ...
    d2 = c2 + 2.0*(COMP_PRECISION)n*plate_h;
    fam = atan2(dx,d1) - atan2(dx,d2);
    /* second image family, see ve_rybicki_antiplane_uz */
    d1 = 2.0*(COMP_PRECISION)n*plate_h - c2;
    d2 = 2.0*(COMP_PRECISION)n*plate_h - c1;
    fam += atan2(dx,d1) - atan2(dx,d2);
    vz += ve_nur_mavko_weight_dot(n,gamma0,b,t) * fam;
  }
  return vz/PI;
}

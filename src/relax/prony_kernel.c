			       ...
			       COMP_PRECISION t_M, int np)
{
  ...
  a = 1.0/(2.0*t_M);
  spec->np = np;
  /* geometric ladder of rates spanning [a/3, 3a], centered on a */
  for(i=0;i < np;i++)
    spec->tau[i] = 1.0/(a * pow(3.0,
				-1.0 + 2.0*(COMP_PRECISION)i/
				((np > 1)?((COMP_PRECISION)(np-1)):(1.0))));
  spec->has_const = TRUE;
  spec->nterm = np + 1;
  spec->ns = spec->nterm + VE_NHELD;
  /* fit samples bracketing the ladder, held-out interleaved */
  for(i=0;i < spec->nterm;i++)
    spec->sk[i] = a * 0.04 * pow(150.0,
				 (COMP_PRECISION)i/
				 ((COMP_PRECISION)spec->nterm - 1.0));
  spec->sk[spec->nterm]   = a * 0.10;
  spec->sk[spec->nterm+1] = a * 1.70;
  ve_solve_weights(spec);
}

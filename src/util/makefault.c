...
#ifdef ALLOW_NON_3DQUAD_GEOM
  fprintf(stderr,"makefault: x: (%g, %g, %g) l: %g w: %g s: %g d: %g grp: %i n: %5i m: %5i patches: %10i scaling: %11e %s\n",
...
#else
  fprintf(stderr,"makefault: x: (%g, %g, %g) l: %g w: %g s: %g d: %g grp: %i n: %5i m: %5i patches: %10i scaling: %11e\n",
	  fault[0].x[INT_X],fault[0].x[INT_Y],fault[0].x[INT_Z],
	  fault[0].l,fault[0].w,fault[0].strike,
	  fault[0].dip,fault[0].group,seg[0],seg[1],nrpatches,CHAR_FAULT_DIM);
#endif

...
`-ve_mode 3` takes the kernel from a FILE instead of an internal
generator: `-ve_prony_file <f>` supplies shared relaxation times and
per-pair amplitude matrices (same K(t) = C_inf + sum_p C_p
exp(-t/tau_p) contract, C_inf implicit), plus a high-k elastic K0
block used only for a consistency gate against the assembled `Is`
(off-diagonal entries; tolerance `-ve_prony_k0tol`, default warn at
0.05 and abort at 5x). This makes the VE machinery
element-agnostic and generator-agnostic: it currently drives 2-D
IN-PLANE (plane-strain, dipping-fault) cycles with kernels for an
elastic plate over a Maxwell half-space WITH GRAVITY, generated by
`test_relax/inplane_ve_proto/bp3_ve_kernels.py`, and is the intended
landing pad for PSGRN/PSCMP-derived 3-D layered
...
t [s], slip [m], log10 slip rate, shear stress [MPa], normal stress
[MPa] and log10 theta [s]; output is every accepted step while the
station slip rate exceeds `-rsf_station_vdense` (default 1e-6 m/s)
and every `-rsf_station_dt_yr` (default 0.1 yr) otherwise, and
appends across restarts. Generators write the station file for
their geometry (see `seas_bp_tests/bp3/gen_bp3.py`).
...
steps and once more after a regular finish; `-rsf_checkpoint_wall <s>`
adds a WALLCLOCK cadence (every s seconds), which is what long runs
usually want since the accepted-step rate varies by orders of
magnitude through a cycle; `-rsf_checkpoint_file`

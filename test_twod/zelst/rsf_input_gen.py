...
geom = "geom_zelst.in"
idxfile = "geom_zelst.in.idx"
group_profiles = {0: "profile_megathrust.dat"}   # every other group:
default_profile = "profile_splay.dat"            # this profile
dc_by_group = {0: 0.02}       # D_c [m] per group; others use dc_default
dc_default = 0.02
dsigma_dz = 18.0e6            # effective normal stress gradient [Pa/km]
sigma_cap = 50.0e6            # cap [Pa]
sigma_min = 2.0e6             # floor for the shallowest patches [Pa]
vpl_by_group = {0: 1.0e-9}    # backslip rate [m/s]; unlisted groups: 0

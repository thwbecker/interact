...
rsf_solve:
	...
	follow HBI and reproduce the SCEC SEAS benchmark BP5 (see bp5
	directory).

	Five state (constitutive) evolution laws are implemented,
	...
	that Omega = 1 is steady state), their theta forms are:
	  ...
	  3  PRZ (Perrin, Rice and Zheng, 1995), normalized so that
	     ...
	     Omega^2 above steady state, which makes PRZ stiff for
	...
	rsf_solve.md; all five laws are implemented in one shared
	routine (rsf_state_rate) used by both the explicit and the
	-imex integrator paths.
...
PetSc support was compiled in, providing limited access to parallel solves for one-step problems.
      ...
      For iterative solve, the recommended setting is the near-field preconditioner (see -near_pc_rfac below):
                          "-near_pc_rfac 6 -ksp_type gmres -pc_type asm -sub_pc_type lu -ksp_norm_type unpreconditioned -ksp_rtol 1e-6"
...
(1) The fault geometry is read from file "geom.in" (unless changed by -geom),
    ...
    - 2D SEGMENTS in the x-y (half) plane:
      ...
      If dip = -90, dislocations are anti-plane and strike means mode III, out of plane motion (deformation in z direction).
...
(2) Boundary conditions and the operational mode are input via the "bc.in" file
 ...
 IF ONE STEP CALCULATION IS CHOSEN, the following input fields should be
   ...
   (A) 0, 1, 2, 3: slip on fault specified (if called several times, will add up)
    ...
    For rake: A mixture between strike (rake angle 0) and dip slip (rake angle 90, counted CCW from strike);
...
PARAMETERS:
 ...
 Stress drop (1) normalization was chosen with default shear modulus 1.000000e+04 and Poisson 0.25
 Note that -nu allow setting the Poisson ratio on run time.
...
OPTIONS:
 ...
 -f  read in fault friction from file "fp.in", so far only parameters for the friction law
 ...
 -rk read in fault rake from file "rake.in"
     OFF by default, if switch is set will be ON.
 ...
 -ni suppress all interactions between faults except the interactions of patches within the fault group
     applies to the A matrix and to the post slip fault stress evaluation (cross group stress not added)

 -geom file_name	assigns different geometry file, default is geom.in
 ...
 -near_pc_radius value   or   -near_pc_rfac factor
     for the Petsc one-step solve, also assemble a sparse matrix from the A entries with
     source-receiver distance < radius and hand it to the KSP as the preconditioning
     matrix. The radius is either given directly (geometry length units) or as factor
     times the mean patch length sqrt(area) (e.g. -near_pc_rfac 4). Use with e.g.
     -pc_type asm -sub_pc_type lu (or ilu); -pc_type jacobi/none ignore it. Off by default.

 -npsfse
     no post slip fault stress evaluation: after a one-step solve, assign
     the slip solution to the patches and print it, but skip the resolved
     stress evaluation on all patches, i.e. flt.dat stresses stay at the
     pre-solve values (default off, i.e. the stress evaluation is performed)

 -spsfse
     force the serial post slip fault stress evaluation (add_solution on every
     rank) in one-step mode, even when running on several MPI ranks

 -ppsfse
     force the parallel post slip fault stress evaluation (receivers split
     over ranks, then gathered) in one-step mode, even on a single rank.
     default is serial on one rank, parallel on several ranks

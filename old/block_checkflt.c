...
read and checks faults in the blockinvert format

run this program before using blockinvert



read in block centroid locations

decide if fault block code assignment should be swapped
...
int main(int argc, char **argv)
{
  ...
  FILE *out;
  int i,j,minus_one=-1,constrain_euler=0;
  ...
  */
  ...
  for(i=0;i < mod->nflt;i++){
    ...
    /*
      ...
      (dir[]) in the definition from above ( dir[j] = -px[INT_Y] / norm(px,2);)

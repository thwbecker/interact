!-------------------------------------------------------------------------------
!
! these routines are from the HBI code by So Ozawa, who he presumably converted
! Mehdi Nikkhoo's Matlab code
! https://github.com/sozawa94/hbi/blob/master/TDstressFS.f90
! only renamed - this all produces identical answers to my conversion down to 1e-6
!
subroutine hbi_tdstresshs(X,Y,Z,P1,P2,P3,Ss,Ds,Ts,mu,lambda,&
  ...
  call hbi_TDstressFS(X,Y,Z,P1,P2,P3,Ss,Ds,Ts,mu,lambda,&
       &Sxx_m,Syy_m,Szz_m,Sxy_m,Sxz_m,Syz_m)

  call hbi_TDstress_HarFunc(X,Y,Z,P1,P2,P3,Ss,Ds,Ts,mu,lambda,&
  ...
  Q2(3)=-Q2(3)
  Q3(3)=-Q3(3)
  call hbi_TDstressFS(xx,yy,zz,Q1,Q2,Q3,Ss,Ds,Ts,mu,lambda,&
...
end subroutine hbi_TDstressHS

subroutine hbi_TDstressFS(X,Y,Z,P1,P2,P3,Ss,Ds,Ts,mu,lambda,&
     & Sxx,Syy,Szz,Sxy,Sxz,Syz)
...
!!!write(*,*) 'calc'
  ...
  P3mP1 = P3 - P1
  CALL hbi_DCross(P2mP1, P3mP1, Vnorm) ! but, this still needs to be normalized:
  Vnorm=Vnorm/sqrt(dot_product(Vnorm,Vnorm))

  eY = (/ 0.0D0, 1.0D0, 0.0D0 /)
  eZ = (/ 0.0D0, 0.0D0, 1.0D0 /)
  CALL hbi_DCross(eZ, Vnorm, Vstrike)
  ...
  END IF
  Vstrike=Vstrike/sqrt(dot_product(Vstrike,Vstrike))
  CALL hbi_DCross(Vnorm, Vstrike, Vdip)
  p_1 = 0.0D0
  p_2 = 0.0D0
  p_3 = 0.0D0
  ...
  CALL hbi_CoordTrans(X-P2(1), Y-P2(2), Z-P2(3), At, & ! inputs
       & x_, y_, z_)                      ! outputs
  CALL hbi_CoordTrans(P1(1)-P2(1), P1(2)-P2(2), P1(3)-P2(3), At, & ! inputs
       & p_1(1), p_1(2), p_1(3))                      ! outputs
  CALL hbi_CoordTrans(P3(1)-P2(1), P3(2)-P2(2), P3(3)-P2(3), At, & ! inputs
...
!!!write(*,*) At
  ...
  CALL hbi_trimodefinder(y_, z_, x_, p_1(2:3), p_2(2:3), p_3(2:3), & ! inputs
       & Trimode)
  ...
  IF (casepLog) THEN
     CALL hbi_SetupD(xp, yp, zp, A, bx, by, bz, nu, p_1, -e13, & ! inputs
          & Exx1Tp,Eyy1Tp,Ezz1Tp,Exy1Tp,Exz1Tp,Eyz1Tp)
     CALL hbi_SetupD(xp, yp, zp, B, bx, by, bz, nu, p_2, e12, & ! inputs
          & Exx2Tp,Eyy2Tp,Ezz2Tp,Exy2Tp,Exz2Tp,Eyz2Tp)
     CALL hbi_SetupD(xp, yp, zp, C, bx, by, bz, nu, p_3, e23, & ! inputs
          & Exx3Tp,Eyy3Tp,Ezz3Tp,Exy3Tp,Exz3Tp,Eyz3Tp)
  END IF
  
  IF (caseNLog) THEN
     CALL hbi_SetupD(xn, yn, zn, A, bx, by, bz, nu, p_1, e13, & ! inputs
          & Exx1Tn,Eyy1Tn,Ezz1Tn,Exy1Tn,Exz1Tn,Eyz1Tn)
     CALL hbi_SetupD(xn, yn, zn, B, bx, by, bz, nu, p_2, -e12, & ! inputs
          & Exx2Tn,Eyy2Tn,Ezz2Tn,Exy2Tn,Exz2Tn,Eyz2Tn)
     CALL hbi_SetupD(xn, yn, zn, C, bx, by, bz, nu, p_3, -e23, & ! inputs
  ...
  !write(*,*)At
  call hbi_TensTrans(exx,eyy,ezz,exy,exz,eyz,At,&
...
end subroutine hbi_TDstressFS
subroutine hbi_TDstress_HarFunc(X,Y,Z,P1,P2,P3,Ss,Ds,Ts,mu,lambda,&
    &Sxx,Syy,Szz,Sxy,Sxz,Syz)
  implicit none
  real(8),intent(in)::X,Y,Z,P1(3),P2(3),P3(3),Ss,Ds,Ts,mu,lambda
  real(8),intent(out)::Sxx,Syy,Szz,Sxy,Sxz,Syz
  ...
  P3mP1 = P3 - P1
  !write(*,*) P2mP1,P3mP1
  CALL hbi_DCross(P2mP1, P3mP1, Vnorm)
  Vnorm=Vnorm/sqrt(dot_product(Vnorm,Vnorm))
  !write(*,*)Vnorm
  eY = (/ 0.0D0, 1.0D0, 0.0D0 /)
  eZ = (/ 0.0D0, 0.0D0, 1.0D0 /)
  ...
  At(1:3, 3) = Vdip(1:3)
  CALL hbi_CoordTrans(bx, by, bz, At, & ! inputs
       & bxt, byt, bzt)

  call hbi_AngSetupFSC_S(X,Y,Z,bXt,bYt,bZt,P1,P2,mu,lambda,&
       &Sxx1,Syy1,Szz1,Sxy1,Sxz1,Syz1)
  !write(*,*)Sxx1,Syy1,Szz1,Sxy1,Sxz1,Syz1
  ! write(*,*)
  call hbi_AngSetupFSC_S(X,Y,Z,bXt,bYt,bZt,P2,P3,mu,lambda,&
       &Sxx2,Syy2,Szz2,Sxy2,Sxz2,Syz2)
  !write(*,*)Sxx2,Syy2,Szz2,Sxy2,Sxz2,Syz2
  ! write(*,*)
  call hbi_AngSetupFSC_S(X,Y,Z,bXt,bYt,bZt,P3,P1,mu,lambda,&
  ...
  return
end subroutine hbi_TDstress_HarFunc
  !------------------------------------------------------------------------------
subroutine hbi_TensTrans(Txx1,Tyy1,Tzz1,Txy1,Txz1,Tyz1,A,&
...
end subroutine hbi_TensTrans
!------------------------------------------------------------------------------
SUBROUTINE hbi_CoordTrans(x1, x2, x3, A, &
     & Xcap1, Xcap2, Xcap3)
  IMPLICIT NONE
...
END SUBROUTINE Hbi_CoordTrans
  !------------------------------------------------------------------------------
SUBROUTINE hbi_trimodefinder(x, y, z, p1, p2, p3, & ! inputs
     & trimode)               ! output
  IMPLICIT NONE
...
end subroutine hbi_trimodefinder
  !------------------------------------------------------------------------------
SUBROUTINE hbi_SetupD(x, y, z, alpha, bx, by, bz, nu, TriVertex, SideVec, & ! inputs
  ...
  call hbi_AngDisStrain(x, y1, z1, -pi+alpha, bx, by1, bz1, nu, & ! inputs
       & Exx,Eyy,Ezz,Exy,Exz,Eyz) ! outputs
  ...
  exz_=exz
  eyz_=eyz
  call hbi_TensTrans(exx_,eyy_,ezz_,exy_,exz_,eyz_,B,& !inputs
       & Exx,Eyy,Ezz,Exy,Exz,Eyz) !outputs
  return
end subroutine hbi_SetupD
! function [Stress,Strain]=AngSetupFSC_S(X,Y,Z,bX,bY,bZ,PA,PB,mu,lambda)
subroutine hbi_AngSetupFSC_S(X,Y,Z,bX,bY,bZ,PA,PB,mu,lambda,Sxx,Syy,Szz,Sxy,Sxz,Syz)
  implicit none
  ...
  if ((abs(beta)<1d-3).or.(abs(pi-beta)<1d-3)) then
     ...
     CALL hbi_CoordTrans(X-PA(1),Y-PA(2),Z-PA(3), A, & ! inputs
          & y1A,y2A,y3A)
  ...
  !write(*,*)'y1A,y2A,y3A',y1A,y2A,y3A
     ...
     CALL hbi_CoordTrans(SideVec(1),SideVec(2),SideVec(3), A, & ! inputs
          & y1AB,y2AB,y3AB)
  ...
  !write(*,*) 'y1B,y2B,y3B',y1B,y2B,y3B
     ...
     CALL hbi_CoordTrans(bx, by, bz, A, & ! inputs
          & b1, b2, b3)
     ...
     if(configI) then
        call hbi_AngDisStrainFSC(y1A,y2A,y3A,-pi+beta,b1,b2,b3,nu,-PA(3),&
        ...
        call hbi_AngDisStrainFSC(y1B,y2B,y3B,-pi+beta,b1,b2,b3,nu,-PB(3),&
        ...
        call hbi_AngDisStrainFSC(y1A,y2A,y3A,beta,b1,b2,b3,nu,-PA(3),&
             &v11A,v22A,v33A,v12A,v13A,v23A)
        !write(*,*)'v11A,v22A,v33A,v12A,v13A,v23A'
        !write(*,*)v11A,v22A,v33A,v12A,v13A,v23A
        !
        ...
        call hbi_AngDisStrainFSC(y1B,y2B,y3B,beta,b1,b2,b3,nu,-PB(3),&
             &v11B,v22B,v33B,v12B,v13B,v23B)
        !write(*,*)'v11B,v22B,v33B,v12B,v13B,v23B'
        !write(*,*)v11B,v22B,v33B,v12B,v13B,v23B
     end if
     ...
     call hbi_TensTrans(v11,v22,v33,v12,v13,v23,A,&
...
end subroutine hbi_AngSetupFSC_S
  !------------------------------------------------------------------------------
SUBROUTINE hbi_AngDisStrain(x, y, z, alpha, bx, by, bz, nu, & ! inputs
     & Exx,Eyy,Ezz,Exy,Exz,Eyz) ! outputs
...
end subroutine hbi_AngDisStrain

subroutine hbi_AngDisStrainFSC(y1,y2,y3,beta,b1,b2,b3,nu,a,&
...
end subroutine hbi_AngDisStrainFSC
subroutine hbi_DCross(v1,v2,v3)
  implicit none
